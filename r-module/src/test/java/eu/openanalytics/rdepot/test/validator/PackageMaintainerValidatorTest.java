...
public class PackageMaintainerValidatorTest {
	...
	public void validatePackageMaintainer_nullPackageAndNullUserAndNullRepository() throws Exception {
		...
		verify(errors, times(1)).rejectValue("packageName", RefactoredMessageCodes.EMPTY_PACKAGE, null, null);
		verify(errors, times(1)).rejectValue("user", RefactoredMessageCodes.EMPTY_USER);
		verify(errors, times(1)).rejectValue("repository", RefactoredMessageCodes.REPOSITORY_NOT_FOUND);
	}
	...
	public void validatePackageMaintainer_duplicateNewMaintainer() throws Exception {
		...
		when(packageMaintainerService.findByPackageAndRepositoryAndNonDeleted(
				packageMaintainer.getPackageName(), packageMaintainer.getRepository()))
		.thenReturn(Optional.ofNullable(packageMaintainer));
	...
	@Test
	public void validatePackageMaintainer_duplicateDifferentMaintainer() throws Exception {
		packageMaintainerValidator = new PackageMaintainerValidator(userService, repositoryService, packageMaintainerService);

		RRepository repository = RRepositoryTestFixture.GET_EXAMPLE_REPOSITORY();
		PackageMaintainer packageMaintainer = PackageMaintainerTestFixture.GET_PACKAGE_MAINTAINER_FOR_REPOSITORY(repository);			
		PackageMaintainer updatedPackageMaintainer = new PackageMaintainer(packageMaintainer);
		updatedPackageMaintainer.setId(111);
		...
		when(packageMaintainerService.findByPackageAndRepositoryAndNonDeleted(
				packageMaintainer.getPackageName(), packageMaintainer.getRepository()))
		.thenReturn(Optional.ofNullable(updatedPackageMaintainer));
