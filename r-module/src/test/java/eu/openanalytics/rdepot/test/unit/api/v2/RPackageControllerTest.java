...
public class RPackageControllerTest extends ApiV2ControllerUnitTest {
	...
	public void getAllPackages_returns401_whenUserIsNotAuthenticated() throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/v2/manager/r/packages").contentType(MediaType.APPLICATION_JSON))
	...
	public void getPackage_returns401_whenUserIsNotAuthenticated() throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/v2/manager/r/packages/" + 123).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isUnauthorized())
				.andExpect(content().json(Files.readString(Path.of(ERROR_NOT_AUTHENTICATED_PATH))));
	}
	...
	public void getAllDeletedPackages_returns403_whenUserIsNotAdmin() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get("/api/v2/manager/r/packages").param("deleted", "true")
	...
	public void getAllPackages() throws Exception {
		...
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/v2/manager/r/packages/")
	...
	public void getAllPackages_WhenRepositoryIsSpecified() throws Exception {
		...
		when(rRepositoryService.findByName(repository.getName())).thenReturn(Optional
				.of(eu.openanalytics.rdepot.r.test.strategy.fixture.RRepositoryTestFixture.GET_EXAMPLE_REPOSITORY()));
		when(userService.findByLogin("user")).thenReturn(user);
	...
	public void getPackageById_AsAdmin() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/api/v2/manager/r/packages/" + packageBag.get().getId())
				.contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(content().json(Files.readString(Path.of(EXAMPLE_PACKAGE_PATH))));
	}

	// TODO returns all packages - even deleted, in controller this is also in todo,
	...
	public void getPackageById_returns404_WhenPackageIsNotFound() throws Exception {
		...
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/v2/manager/r/packages/" + ID).contentType(MediaType.APPLICATION_JSON))
	...
	public void patchPackage_returns404_WhenPackageIsNotFound() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.patch("/api/v2/manager/r/packages/" + ID).content(patchJson)
	...
	public void patchPackage_returns401_WhenUserIsNotAuthenticated() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.patch("/api/v2/manager/r/packages/" + 123).content(patchJson)
	...
	public void patchPackage() throws Exception {
		...
		Strategy<RPackage> strategy = Mockito
				.spy(new SuccessfulStrategy<RPackage>(packageBag, newsfeedEventService, rPackageService, user.get()));
		when(rPackageValidator.supports(any())).thenReturn(true);
		when(rStrategyFactory.updatePackageStrategy(any(), eq(user.get()), any())).thenReturn(strategy);
		when(userService.findById(any())).thenReturn(user);
		when(rRepositoryService.findById(any())).thenReturn(Optional.of(repository));
		when(rPackageService.findById(packageBag.getId())).thenReturn(Optional.of(packageBag));
		when(submissionService.findById(any())).thenReturn(Optional.of(submission));
		when(userService.findByLogin("user")).thenReturn(user);
	...
	public void patchPackage_returns422_whenValidationFails() throws Exception {
		...
		doThrow(new PackageValidationException("")).when(rPackageValidator).validate(any(), eq(true), any());
	...
	public void patchPackage_returns500_whenErrorIsThrown() throws Exception {
		...
		Submission submission = RSubmissionTestFixture.GET_FIXTURE_SUBMISSION(user.get(), packageBag);
		Strategy<RPackage> strategy = Mockito
				.spy(new FailureStrategy<RPackage>(packageBag, newsfeedEventService, rPackageService, user.get()));
	 ...
	 packageBag.setDeleted(true);
		...
		when(userService.findByLogin("user")).thenReturn(user);
		doThrow(new DeleteEntityException()).when(rPackageDeleter).delete(packageBag);
		when(rPackageService.findById(packageBag.getId())).thenReturn(Optional.of(packageBag));
		
	 when(rPackageService.findOneDeleted(packageBag.getId()))
