...
public class PackageTestFixture {
	...
	public static List<RPackage> GET_FIXTURE_PACKAGES(RRepository repository, User user, int packageCount) {
	...
	public static List<RPackage> GET_FIXTURE_PACKAGES_MULTIPLE_VERSIONS(
			RRepository repository, User user, int packageCount, int versionCount) {
		List<RPackage> packages = new ArrayList<>();
		
		for(int i = 0; i < packageCount; i++) {
			for(int k = 0; k < versionCount; k++) {
				RPackage packageBag = PackageTestFixture.GET_FIXTURE_PACKAGE(repository, user);
				packageBag.setId(i);
