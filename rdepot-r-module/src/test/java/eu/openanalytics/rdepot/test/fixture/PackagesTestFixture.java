...
public class PackagesTestFixture {
	...
	public static List<Package> GET_FIXTURE_PACKAGES(RRepository repository, User user, int packageCount, int idShift) {
		List<Package> packages = new ArrayList<>();
		
		for(int i = idShift; i < packageCount + idShift; i++) {
			Package packageBag = new RPackage(
					i,
					repository,
					user,
			...
			Submission submission = RSubmissionTestFixture.GET_FIXTURE_SUBMISSION(user, (RPackage)packageBag);
			submission.setId(i);
			packageBag.setSubmission(submission);
			packageBag.setVersion(VERSION);
			packages.add((Package)packageBag);
		}
		
		return packages;
	}
	
	public static List<PackageDto> GET_FIXTURE_PACKAGES_DTO(RRepository repository, User user) {
		List<Package> packages = GET_FIXTURE_PACKAGES(repository, user, 3, 100);
		List<PackageDto> packagesDto = new ArrayList<PackageDto>();
		packages.forEach(packageBag -> {
			packagesDto.add(new PackageDto(packageBag));
		});
		return packagesDto;
	}
	
	public static Package GET_FIXTURE_PACKAGE(RRepository repository, User user, Integer id) {
		return GET_FIXTURE_PACKAGES(repository, user, 1, id).get(0);
	}
	
	public static Package GET_FIXTURE_PACKAGE(RRepository repository, User user) {
		return GET_FIXTURE_PACKAGE(repository, user, 0);
	}

	public static Page<Package> GET_EXAMPLE_PACKAGES_PAGED(RRepository repository, User user) {
