...
public class UserValidatorTest {
	...
	public void validateUser_allForbiddenUpdates() throws Exception {
		...
		verify(errors, times(1))
			.rejectValue("lastLoggedInOn", MessageCodes.FORBIDDEN_UPDATE);
		verify(errors, times(1))
			.rejectValue("createdOn", MessageCodes.FORBIDDEN_UPDATE);
		verify(errors, times(1))
			.rejectValue("name", MessageCodes.FORBIDDEN_UPDATE);
		verify(errors, times(1))
			.rejectValue("login", MessageCodes.FORBIDDEN_UPDATE);
		verify(errors, times(1))
			.rejectValue("email", MessageCodes.FORBIDDEN_UPDATE);
	}
	...
	public void validateUser_EmptyEmailLoginAndName() throws Exception {
		...
		userValidator.validate(updatedUser, errors);
		verify(errors, times(1))
			.rejectValue("email", MessageCodes.ERROR_EMPTY_EMAIL);
		verify(errors, times(1))
			.rejectValue("login", MessageCodes.ERROR_EMPTY_LOGIN);
		verify(errors, times(1))
			.rejectValue("name", MessageCodes.ERROR_EMPTY_NAME);
	}
	...
	public void validateUser_InvalidEmailAndDuplicateLogin() throws Exception {
		...
		userValidator.validate(updatedUser, errors);
		verify(errors, times(1))
			.rejectValue("email", MessageCodes.ERROR_INVALID_EMAIL);
		verify(errors, times(1))
			.rejectValue("login", MessageCodes.ERROR_DUPLICATE_LOGIN);
	}
	...
	public void validateUser_DuplicateEmail() throws Exception {
		...
		userValidator.validate(updatedUser, errors);
		verify(errors, times(1))
			.rejectValue("email", MessageCodes.ERROR_DUPLICATE_EMAIL);
	}
}
