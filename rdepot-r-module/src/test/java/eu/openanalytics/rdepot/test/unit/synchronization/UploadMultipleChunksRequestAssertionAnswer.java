...
public class UploadMultipleChunksRequestAssertionAnswer extends UploadChunkRequestAssertionAnswer {
	...
	public ResponseEntity<RepoResponse> answer(InvocationOnMock invocation) throws Throwable {
		final List<List<FileSystemResource>> files = packagesToUpload.stream().map(l -> l.stream().map(f-> new FileSystemResource(f)).toList()).toList();
		final List<List<FileSystemResource>> archiveFiles = packagesToUploadToArchive.stream().map(l -> l.stream().map(f-> new FileSystemResource(f)).toList()).toList();
