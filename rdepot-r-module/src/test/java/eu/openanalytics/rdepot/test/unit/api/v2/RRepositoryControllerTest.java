...
public class RRepositoryControllerTest extends ApiV2ControllerUnitTest {
	...
	public void getAllRepositories() throws Exception {
		...
		when(securityMediator.isAuthorizedToEdit(any(RRepository.class), eq(user.get()))).thenReturn(true);
	...
	public void createRepository() throws Exception {
		...
		Strategy<RRepository> strategy = Mockito
				.spy(new SuccessfulStrategy<RRepository>(createdRepository, newsfeedEventService, rRepositoryService, user.get()));
	...
	public void createRepository_returns422_whenRepositoryValidationFails() throws Exception {
		...
		doAnswer(invocation -> {
			...
			return null;
		}).when(rRepositoryValidator).validate(any(), any());
		when(rRepositoryValidator.supports(RRepository.class)).thenReturn(true);
		...
		mockMvc
			.perform(MockMvcRequestBuilders
					...
					.content(exampleJson))
			.andExpect(
					status().isUnprocessableEntity()); // TODO: #32880 Unify validation errors approach and uncomment
//			.andExpect(
//					content().json(Files.readString(Path.of(ERROR_VALIDATION_REPOSITORY_NAME_PATH))));
	...
	public void createRepository_returns500_whenCreationFails() throws Exception {
		...
		Strategy<RRepository> strategy = Mockito
				.spy(new FailureStrategy<RRepository>(newRepository, newsfeedEventService, rRepositoryService, user.get()));
	...
	public void patchRepository() throws Exception {
		...
		final Integer ID = repository.getId();
		Strategy<RRepository> strategy = Mockito
				.spy(new SuccessfulStrategy<RRepository>(repository, newsfeedEventService, rRepositoryService, user.get()));

		final String patchJson = "[{\"op\": \"replace\",\"path\":\"/serverAddress\",\"value\":\"127.0.0.1\"}]";
		when(rRepositoryService.findById(ID)).thenReturn(Optional.of(repository));
		when(userService.findByLogin("user")).thenReturn(user);
		when(securityMediator.isAuthorizedToEdit(any(RRepository.class), eq(user.get()))).thenReturn(true);
		when(rStrategyFactory.updateRepositoryStrategy(any(),eq(user.get()), any())).thenReturn(strategy);
		doNothing().when(rRepositoryValidator).validate(any(), any());
		when(rRepositoryValidator.supports(RRepository.class)).thenReturn(true);
...
//				content().json(Files.readString(Path.of(ERROR_VALIDATION_REPOSITORY_NAME_PATH))));
	...
	public void deleteRepository_returns500_whenRepositoryDeletionFails() throws Exception {
		...
		doThrow(new DeleteEntityException())
			.when(rRepositoryDeleter).delete(any());
		when(userService.findByLogin("user")).thenReturn(user);
		when(userService.isAdmin(user.get())).thenReturn(true);
