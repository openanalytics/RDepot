...
public class PropertiesFromMetaDataFile {
	...
	public void readPropertiesFromAccruedFile() throws Exception {
		Properties properties = new PropertiesParser(new File(ACCRUED_FILE));
		assertEquals(properties.size(), 14);
		assertEquals(properties.getProperty("Description"), "Package for visualizing data quality of partially accruing data.");
	...
	public void readPropertiesFromDataTableFile() throws Exception {
		Properties properties = new PropertiesParser(new File(DATA_TABLE_FILE));
		assertEquals(properties.size(), 20);
		assertEquals(properties.getProperty("Description"), "Fast aggregation of large data (e.g. 100GB in RAM), fast ordered joins, fast add/modify/delete of columns by group using no copies at all, list columns, friendly and fast character-separated-value read/write. Offers a natural and flexible syntax, for faster development.");
		...
		assertEquals(properties.getProperty("URL"), "https://r-datatable.com, https://Rdatatable.gitlab.io/data.table, https://github.com/Rdatatable/data.table\\n");
	...
	public void readPropertiesFromMatrixFile() throws Exception {
		Properties properties = new PropertiesParser(new File(MATRIX_FILE));
		assertEquals(26, properties.size());
		assertEquals(String.join(
			"",
			"A rich hierarchy of sparse and dense matrix classes,\tincluding general, symmetric, triangular, and diagonal matrices\\n",
		...
		assertEquals("https://R-forge.R-project.org/tracker/?atid=294&group_id=61", properties.getProperty("BugReports"));
		...
		assertEquals(String.join(
			"",
			"Douglas Bates [aut] (<https://orcid.org/0000-0001-8316-9503>), Martin Maechler [aut, cre] (<https://orcid.org/0000-0002-8685-9910>),\\n ",
		...
		assertEquals("do also bump src/version.h, inst/include/Matrix/version.h", properties.getProperty("VersionNote"));
	...
	public void readPropertiesFromGgallyFile() throws Exception {
		Properties properties = new PropertiesParser(new File(GGALLY_FILE));
		assertEquals(26, properties.size());
		...
		assertEquals(String.join(
			"",
			"dplyr (>= 1.0.0), tidyr (>= 1.3.0), grDevices, grid, ggstats, gtable (>= 0.2.0), lifecycle, plyr (>= 1.8.3), progress,\\n ",
			"RColorBrewer, rlang, scales (>= 1.1.0), utils, magrittr\\n"),
			properties.getProperty("Imports")
		);
		assertEquals(String.join(
			"",
			"broom (>= 0.7.0), broom.helpers (>= 1.3.0), chemometrics, geosphere (>= 1.5-1), ggforce, Hmisc, igraph (>= 1.0.1),\\n ",
		...
		assertEquals(String.join(
			"",
			"Barret Schloerke [aut, cre], Di Cook [aut, ths],\\n ",
