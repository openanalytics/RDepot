...
public class RLocalStorageTest extends UnitTest {
	...
	public void setUpDirectories() throws Exception {
		...
		ReflectionTestUtils.setField(storage, RLocalStorage.class, "packageUploadDirectory", packageUploadDirectory, File.class);
		ReflectionTestUtils.setField(storage, RLocalStorage.class, "repositoryGenerationDirectory", repositoryGenerationDirectory, File.class);
	...
	public void buildSynchronizeRequestBody() throws Exception {
		...
		final RPackage benchmarkingArchivePackage = 
				RPackageTestFixture.GET_PACKAGE_FOR_REPOSITORY_AND_USER(repository, user);
		benchmarkingArchivePackage.setSource(new File(packageUploadDirectory 
				+ "/repositories/2/71228208/Benchmarking_0.10.tar.gz").getAbsolutePath());
		...
		final RPackage benchmarkingPackage = 
				RPackageTestFixture.GET_PACKAGE_FOR_REPOSITORY_AND_USER(repository, user);
		benchmarkingPackage.setSource(new File(packageUploadDirectory 
				+ "/repositories/2/79878978/Benchmarking_0.31.tar.gz").getAbsolutePath());
		...
		final RPackage beaR = 
				RPackageTestFixture.GET_PACKAGE_FOR_REPOSITORY_AND_USER(repository, user);
		beaR.setSource(new File(packageUploadDirectory 
				+ "/repositories/2/89565416/bea.R_1.0.5.tar.gz").getAbsolutePath());
		beaR.setName("bea.R");
		...
		final PopulatedRepositoryContent content = 
				storage.organizePackagesInStorage(datestamp, packages, latestPackages, 
						archivePackages, repository);
		
		List<String> remoteLatestPackages = List.of("Benchmarking_0.10.tar.gz");
		List<String> remoteArchivePackages = List.of();
		int versionBefore = 4;
		
		final SynchronizeRepositoryRequestBody requestBody = storage.buildSynchronizeRequestBody(content, remoteLatestPackages, remoteArchivePackages, repository, versionBefore + "");
		
		final File latestPackagesFile = new File(repositoryGenerationDirectory + "/2/current/src/contrib/latest/PACKAGES");
		final File archivePackagesFile = new File(repositoryGenerationDirectory + "/2/current/src/contrib/Archive/PACKAGES");
		
		final File packageToUpload1 = new File(packageUploadDirectory 
				+ "/repositories/2/79878978/Benchmarking_0.31.tar.gz");
		final File packageToUpload2 = new File(packageUploadDirectory 
				+ "/repositories/2/89565416/bea.R_1.0.5.tar.gz");
		final File packageToUploadToArchive = new File(packageUploadDirectory 
				+ "/repositories/2/71228208/Benchmarking_0.10.tar.gz");
		
		assertTrue(requestBody.getPackagesToDeleteFromArchive().isEmpty(), 
				"There should be no packages to delete from Archive.");
		assertEquals(1, requestBody.getPackagesToDelete().size(), 
				"There should one package to delete.");
		assertEquals(2, requestBody.getPackagesToUpload().size(), 
				"Too many packages to upload.");
		assertEquals(1, requestBody.getPackagesToUploadToArchive().size(), 
				"Too many packages to upload to archive.");
		assertTrue(containsFile(packageToUpload1, requestBody.getPackagesToUpload()), 
				"There is a missing package to upload.");
		assertTrue(containsFile(packageToUpload2, requestBody.getPackagesToUpload()), 
				"There is a missing package to upload.");
		assertTrue(containsFile(packageToUploadToArchive, requestBody.getPackagesToUploadToArchive()),
				"There is a missing package to upload to Archive.");
		assertEquals(latestPackagesFile.getAbsolutePath(), requestBody.getPackagesFile().getAbsolutePath(), 
				"Incorrect latest PACKAGES file in the request body");
		assertEquals(archivePackagesFile.getAbsolutePath(), requestBody.getPackagesFileArchive().getAbsolutePath(), 
				"Incorrect archive PACKAGES file in the request body.");
		assertTrue(requestBody.getPackagesToDelete().contains("Benchmarking_0.10.tar.gz"), 
	...
	public void organizePackagesInStorage() throws Exception {
		...
		final File actualLatestPackagesFile = new File(repositoryGenerationDirectory + "/2/20240111/src/contrib/PACKAGES");
		final File actualArchivePackagesFile = new File(repositoryGenerationDirectory + "/2/20240111/src/contrib/Archive/PACKAGES");
		...
		assertEquals(expectedLatestPackagesFileContent, actualLatestPackagesFileContent, "Incorrect latest PACKAGES file");
		assertEquals(expectedArchivePackagesFileContent, actualArchivePackagesFileContent, "Incorrect archive PACKAGES file");
	...
	private String getExpectedArchivePackagesFile() {
		return "Package: Benchmarking\n"
				+ "Version: 0.10\n"
				+ "Depends: Some packages it depends on\n"
	...
	private void verifyPackage(File currentGeneratedDirectory, String filename, boolean archive) {
		final File packageFile = new File(currentGeneratedDirectory + "/src/contrib" + (archive ? "/Archive/" : "/") + filename);
	...
	public void moveToMainDirectory() throws Exception {
		...
		repository.setId(2);
		final User user = UserTestFixture.GET_FIXTURE_USER_PACKAGEMAINTAINER();
		final File packageFileToMove = new File(
				"src/test/resources/unit/storage_tests/rdepot/new/51328701/abc_1.0.tar.gz");
		final byte[] fileContent = Files.readAllBytes(packageFileToMove.toPath());
		final RPackage packageToMove = RPackageTestFixture.GET_PACKAGE_FOR_REPOSITORY_AND_USER(repository, user);
	...
	public void removesGenerationDirectoryContent() throws Exception {
		...
		assertTrue(repositoryGenerationDirectory.listFiles().length == 0, 
				"Generation directory should be empty when shapshots are off.");
	}
	
	@Test
	public void writeToWaitingRoom() throws Exception {
		final File newPackage = 
				new File("src/test/resources/unit/test_packages/accrued_1.2.tar.gz");
		final RRepository repository = RRepositoryTestFixture.GET_EXAMPLE_REPOSITORY();
		repository.setId(2);
	...
	public void moveToTrashDirectory() throws Exception {
		...
		assertTrue(extractedDir.isDirectory() 
				&& extractedDir.getAbsolutePath().endsWith("bea.R"), 
