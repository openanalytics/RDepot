...
cv4abc <- function(param, sumstat, abc.out = NULL, nval, tols,
  ...
  for(mytol in tols){
    ...
    for(i in 1:nval){
      ...
      subres <- abc(target = mytarget, param = myparam, sumstat = mysumstat, tol=mytol,
                    subset = mysubset, method = method, transf = transf, logit.bounds = logit.bounds, kernel = kernel)
  ...
  cv4abc.out <- list(calls = mycall, cvsamples = cvsamp, tols = tols, true = param[cvsamp,], estim = alltol,
