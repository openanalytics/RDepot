...
public class RPackageValidator implements PackageValidator<RPackage> {
	...
	public RPackageValidator(SubmissionService submissionService,
							 RPackageService rPackageService, Environment environment) {
	...
	public void validateUploadPackage(
			RPackage packageBag, boolean replace,
			DataSpecificValidationResult<Submission> validationResult) {
		validateName(packageBag.getName(), validationResult);
		validateNotEmpty("description", packageBag.getDescription(),
				MessageCodes.EMPTY_DESCRIPTION, validationResult);
		validateNotEmpty("author", packageBag.getAuthor(),
				MessageCodes.EMPTY_AUTHOR, validationResult);
		validateNotEmpty("license", packageBag.getLicense(),
				MessageCodes.EMPTY_LICENSE, validationResult);
		validateNotEmpty("title", packageBag.getTitle(),
				MessageCodes.EMPTY_TITLE, validationResult);
		validateNotEmpty("md5sum", packageBag.getMd5sum(),
				MessageCodes.EMPTY_MD5SUM, validationResult);
	...
	private void validateVersion(RPackage packageBag, boolean replace, DataSpecificValidationResult<Submission> validationResult) {
	...
	private void validateFilename(MultipartFile multipartFile, ValidationResult validationResult) {
		...
		String[] tokens = Objects.requireNonNull(multipartFile.getOriginalFilename()).split("_");
