...
public abstract class ApiV2Controller<E extends Resource, D extends IDto> extends ApiV2ReadingController<E,D> {
	 ...
	 * @return full response DTO with 201 http status code
	 */
	protected @ResponseBody ResponseEntity<ResponseDto<EntityModel<D>>> handleCreatedForSingleEntity(E data, User user) {
	 ...
	 */
	protected @ResponseBody ResponseEntity<ResponseDto<EntityModel<D>>> handleWarningForSingleEntity(E data, String messageCode, User user, boolean created) {
		EntityModel<D> model = modelAssembler.toModel(data, user);
		final HttpStatus httpStatus = created ? HttpStatus.CREATED : HttpStatus.OK;

		return ResponseEntity.status(httpStatus)
				.body(ResponseDto.generateWarningBody(messageSource, locale,
						httpStatus, messageCode, model));
	}
	 ...
	 */
	protected @ResponseBody ResponseEntity<ResponseDto<List<String>>>
		handleValidationError(BindingResult bindingResult) {
		final List<String> errorMessages = bindingResult.getAllErrors().stream()
				.map(err -> messageSource.getMessage(
                        Objects.requireNonNull(err.getCode()), null, err.getCode(), locale))
	 ...
	 */
	protected @ResponseBody ResponseEntity<ResponseDto<List<String>>>
		handleValidationError(ValidationResult validationResult) {
	 ...
	 * @return ResponseDto with error related information.
	 */
	protected @ResponseBody ResponseEntity<ResponseDto<List<String>>> handleValidationErrorWithDefaultMessage(BindingResult bindingResult) {
