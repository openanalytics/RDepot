...
public class ResponseDto<T> {
     ...
     */
    public static <T> ResponseDto<T> generateSuccessBody(MessageSource messageSource, Locale locale, T model) {
    ...
    public static ResponseDto<?> generateErrorBody(ApiException e) {
        return e.getHttpStatus().equals(HttpStatus.INTERNAL_SERVER_ERROR)
                ? generateInternalServerErrorBody(e)
                : new ResponseDto<>(
                        Status.ERROR,
    ...
    private static ResponseDto<Map<String, String>> generateInternalServerErrorBody(ApiException e) {
        ...
        return new ResponseDto<>(Status.ERROR, e.getHttpStatus().value(), e.getMessage(), e.getMessageCode(), body);
    }
    ...
    public static ResponseDto<?> generateErrorBody(UnrecognizedQueryParameterException e) {
        return new ResponseDto<>(
                Status.ERROR, e.getHttpStatus().value(), e.getMessage(), e.getMessageCode(), e.getParameters());
    }
     ...
     */
    public static <T> ResponseDto<T> generateErrorBody(
            MessageSource messageSource, Locale locale, HttpStatus httpStatus, String messageCode, T data) {
        String message = messageSource.getMessage(messageCode, null, messageCode, locale);
     ...
     * @param <T> type of additional data to attach
     */
    public static <T> ResponseDto<T> generateWarningBody(
            MessageSource messageSource, Locale locale, HttpStatus httpStatus, String messageCode, T data) {
     ...
     */
    public static <T> ResponseDto<T> generateErrorBody(
            HttpStatus httpStatus, String message, String messageCode, T data) {
        return new ResponseDto<>(Status.ERROR, httpStatus.value(), message, messageCode, data);
    }
}
