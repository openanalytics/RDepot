 ...
 */
@Slf4j
public abstract class YamlDeclarativeConfigurationSource<R extends MirroredRepository<P, M>, 
	P extends MirroredPackage,
	M extends Mirror<P>> implements DeclarativeConfigurationSource<R, P, M> {
	...
	public YamlDeclarativeConfigurationSource(DeclaredRepositoryDirectoriesProps declaredRepositoryDirectoriesProps,
	...
	protected List<R> retrieveRepositoriesFromDirectory(String directoryPath) {
		...
		for(File configFile : filesInDir) {
			if(configFile.getName().endsWith("repository.yaml") 
				...
				try {
					declaredRepositories.add(
							retrieveDeclaredRepositoryFromFile(configFile)
					);
