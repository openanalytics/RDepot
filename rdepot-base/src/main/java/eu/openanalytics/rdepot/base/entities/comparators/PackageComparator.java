...
public class PackageComparator<P extends Package> implements Comparator<P> {

    @Override
    public int compare(P packageA, P packageB) {

        if (packageA.getRepository().getId() != packageB.getRepository().getId())
            return packageA.getRepository()
                    .getName()
                    .compareTo(packageB.getRepository().getName());
