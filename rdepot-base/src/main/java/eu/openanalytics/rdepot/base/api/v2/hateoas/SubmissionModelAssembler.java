 ...
 */
@Component
public class SubmissionModelAssembler extends AbstractRoleAwareModelAssembler<Submission, SubmissionDto> {
    ...
    @Autowired
    public SubmissionModelAssembler(
    ...
    private SubmissionModelAssembler(
            DtoConverter<Submission, SubmissionDto> dtoConverter,
            PackageModelAssembler packageModelAssembler,
            SecurityMediator securityMediator,
    ...
    @Override
    protected List<Link> generateAvailableLinksForEntity(Submission entity, Class<?> extensionControllerClass) {
        Class<?> controllerClass = submissionControllerClassesByTechnology.getOrDefault(
                entity.getTechnology(), ApiV2SubmissionController.class);

        return List.of(
                linkTo(controllerClass).slash(entity.getId()).withSelfRel(),
                linkTo(ApiV2SubmissionController.class).withRel(resourceTypeName + "List"));
    }
    ...
    public @NonNull EntityModel<SubmissionDto> toModel(@NonNull Submission entity) {
        ...
        Objects.requireNonNull(model.getContent()).setPackageBag(packageModelAssembler.toModel(entity.getPackage()));
        return model;
    }
    ...
    public RepresentationModelAssembler<Submission, EntityModel<SubmissionDto>> assemblerWithUser(User user) {
        return new SubmissionModelAssembler(
                dtoConverter, packageModelAssembler, securityMediator, submissionControllerClassesByTechnology, user);
    }

    @Override
    protected Class<?> getExtensionControllerClass(Submission entity) {
        return submissionControllerClassesByTechnology.getOrDefault(
                entity.getTechnology(), ApiV2SubmissionController.class);
    }
}
