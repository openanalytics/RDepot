...
public class ApiV2RepositoryController 
	extends ApiV2ReadingController<Repository, RepositoryDto> {
	...
	public ApiV2RepositoryController(MessageSource messageSource, 
	...
	public @ResponseBody ResponseDto<PagedModel<EntityModel<RepositoryDto>>> getAllRepositories(
		...
		Specification<Repository> specification = SpecificationUtils.andComponent(null,
				RepositorySpecs.isDeleted(deleted));
		...
		if(published.isPresent()) {
			specification = SpecificationUtils.andComponent(specification, RepositorySpecs.isPublished(published.get()));
		}
		...
		if(search.isPresent()) {
			specification = SpecificationUtils
				.andComponent(specification, RepositorySpecs.ofNameSearching(search.get()));
		}
		
		return handleSuccessForPagedCollection(repositoryService.findAllBySpecification(specification, resolvedPageable), requester);
	}
