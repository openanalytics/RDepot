...
public abstract class AbstractRoleAwareModelAssembler<T extends Resource, D extends IDto>
	...
	protected List<Link> generateRoleBasedAvailableLinksForEntity(T entity, User user) {
        List<Link> links = new ArrayList<>(
				generateAvailableLinksForEntity(entity,
                	getExtensionControllerClass(entity)
				)
		);
		
		final Link baseSelfLink = linkTo(getExtensionControllerClass(entity)).slash(entity.getId()).withSelfRel();
	 ...
	 */
	protected List<Link> generateAvailableLinksForEntity(T entity, 
			Class<?> extensionControllerClass) {
		return List.of(linkTo(extensionControllerClass).slash(entity.getId()).withSelfRel(),
