...
public class EmailServiceImpl implements EmailService {
	...
	protected void handleEmailDisabled() {
		logger.debug("E-Mail service has been disabled by administrator.");
	}
	...
	public void sendAcceptSubmissionEmail(final Submission submission) {
		...
		final String submissionUrl = ServletUriComponentsBuilder
				.fromCurrentContextPath()
				.path("/manager/submissions/{id}")
				.buildAndExpand(submission.getId())
				.toUri()
				.toString(); //TODO: which version of the API do we refer to here?
		
		final String subject = "RDepot: new submission";
		final String message = "Dear " + to.getName() + ",<br/><br/>" 
				+ "There's a new submission from " + submission.getUser().getName()
				+ ": " + submission.getPackage().getName() + " " + submission.getPackage().getVersion()
				...
				+ "To get an <strong>overview</strong> of the submission in more detail, please use <a href='" + submissionUrl + "'>this</a> link.<br/>"
				+ "To <strong>cancel</strong> the submission directly, please use <a href='" + submissionUrl + "/cancel'>this</a> link.<br><br>"
				+ "Thank you for your cooperation.<br/><br/>"
				+ "Sincerely yours,<br/>"
				+ "The RDepot team";
		
		logger.debug("Sending accept submission e-mail");
		sendEmail(emailFrom, to.getEmail(), subject, message);
	}
	
	protected void sendEmail(final String emailFrom, final String emailTo, 
	...
	public void sendCancelledSubmissionEmail(final Submission submission) {
		...
		final String message = "Dear " + to.getName() + ",<br/><br/>"
				+ "The submission from " + submission.getUser().getName()
