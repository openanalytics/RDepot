...
public class KeycloakCustomBindAuthenticator extends CustomBindAuthenticator {
	
	private final Environment environment;
	
	@Autowired
	public KeycloakCustomBindAuthenticator(Environment environment, 
			UserService userService, RoleService roleService,
			SecurityMediator securityMediator) {
		super(environment, userService, roleService, securityMediator, "keycloak");
		this.environment = environment;
	}

	public Collection<? extends GrantedAuthority> authenticate(String username, AccessToken token) throws AuthException {
		return authenticate(username, token.getEmail(), NameMapper.getName(environment, token));
	}
}
