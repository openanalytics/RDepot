...
import eu.openanalytics.rdepot.test.fixture.PackagesTestFixture;
...
public class PackagesControllerTest extends ApiV2ControllerUnitTest {
	...
	public static final String EXAMPLE_PACKAGES_PATH = JSON_PATH + "/example_common_packages.json";
	public static final String EXAMPLE_PACKAGE_PATH = JSON_PATH + "/example_common_package.json";
	...
	public void initEach() {
		user = Optional.of(UserTestFixture.GET_FIXTURE_USER());
	}
	...
	public void getPackage() throws Exception {
		final Package packageBag =  PackagesTestFixture.GET_EXAMPLE_PACKAGE();
		final Integer ID = 1234;
		final PackageDto packageDto = PackagesTestFixture.GET_EXAMPLE_PACKAGE_DTO();
		...
		mockMvc
			.perform(MockMvcRequestBuilders
					.get("/api/v2/manager/packages/" + ID)
					.contentType(MediaType.APPLICATION_JSON))
	...
	public void getPackage_returns401_whenUserIsNotAuthenticated() throws Exception {
		mockMvc
			.perform(MockMvcRequestBuilders
					.get("/api/v2/manager/packages/" + 1234)
					.contentType(MediaType.APPLICATION_JSON))
			.andExpect(status().isUnauthorized())
			.andExpect(content().json(Files.readString(Path.of(ERROR_NOT_AUTHENTICATED_PATH))));
	}
	...
	public void getPackage_returns404_whenRepositoryIsNotFound() throws Exception {
		...
		when(userService.findByLogin("user")).thenReturn(UserTestFixture.GET_FIXTURE_ADMIN());
		
		mockMvc
			.perform(MockMvcRequestBuilders
					.get("/api/v2/manager/packages/" + 123)
					.contentType(MediaType.APPLICATION_JSON))
