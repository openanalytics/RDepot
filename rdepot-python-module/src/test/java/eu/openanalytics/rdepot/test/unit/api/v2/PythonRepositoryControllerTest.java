...
public class PythonRepositoryControllerTest extends ApiV2ControllerUnitTest {
	...
	public void getAllRepositories() throws Exception {
		...
		when(userService.findByLogin("user"))
		.thenReturn(user);
		...
		when(securityMediator.isAuthorizedToEdit(any(PythonRepository.class), eq(user.get()))).thenReturn(true);
		
		mockMvc
			.perform(MockMvcRequestBuilders
					.get("/api/v2/manager/python/repositories")
	...
	public void getRepository() throws Exception {
		...
		mockMvc
			.perform(MockMvcRequestBuilders
					.get("/api/v2/manager/python/repositories/" + ID)
					.contentType(MediaType.APPLICATION_JSON))
	...
	public void getRepository_returns401_whenUserIsNotAuthenticated() throws Exception {
		mockMvc
			.perform(MockMvcRequestBuilders
					.get("/api/v2/manager/python/repositories/" + 123)
					.contentType(MediaType.APPLICATION_JSON))
			.andExpect(status().isUnauthorized())
			.andExpect(content().json(Files.readString(Path.of(ERROR_NOT_AUTHENTICATED_PATH))));
	}
	...
	public void createRepository() throws Exception {
		...
		Strategy<PythonRepository> strategy = Mockito
				.spy(new SuccessfulStrategy<PythonRepository>(createdRepository, newsfeedEventService, pythonRepositoryService, user.get()));

		when(pythonStrategyFactory.createRepositoryStrategy(any(), eq(user.get()))).thenReturn(strategy);
	...
	public void createRepository_returns422_whenRepositoryValidationFails() throws Exception {
		...
		doAnswer(invocation -> {
			...
			return null;
		}).when(pythonRepositoryValidator).validate(any(), any());
		when(pythonRepositoryValidator.supports(PythonRepository.class)).thenReturn(true);
	...
	public void createRepository_returns500_whenCreationFails() throws Exception {
		...
		Strategy<PythonRepository> strategy = Mockito
				.spy(new FailureStrategy<PythonRepository>(newRepository, newsfeedEventService, pythonRepositoryService, user.get()));
	...
	public void patchRepository() throws Exception {
		...
		final Integer ID = repository.getId();
		Strategy<PythonRepository> strategy = Mockito
				.spy(new SuccessfulStrategy<PythonRepository>(repository, newsfeedEventService, pythonRepositoryService, user.get()));

		final String patchJson = "[{\"op\": \"replace\",\"path\":\"/serverAddress\",\"value\":\"127.0.0.1\"}]";
		when(pythonRepositoryService.findById(ID)).thenReturn(Optional.of(repository));
		when(userService.findByLogin("user")).thenReturn(user);
		when(securityMediator.isAuthorizedToEdit(any(PythonRepository.class), eq(user.get()))).thenReturn(true);
		when(pythonStrategyFactory.updateRepositoryStrategy(any(),eq(user.get()), any())).thenReturn(strategy);
		doNothing().when(pythonRepositoryValidator).validate(any(), any());
		when(pythonRepositoryValidator.supports(PythonRepository.class)).thenReturn(true);
	...
	public void patchRepository_returns401_whenUserIsNotAuthenticated() throws Exception {
		...
		mockMvc
			.perform(MockMvcRequestBuilders
				.patch("/api/v2/manager/python/repositories/" + 123)
				.contentType("application/json-patch+json")
				.content(patchJson))
			.andExpect(status().isUnauthorized())
			.andExpect(content().json(Files.readString(Path.of(ERROR_NOT_AUTHENTICATED_PATH))));
	...
	public void patchRepository_returns403_whenUserIsNotAuthorized() throws Exception {
		...
		when(userService.findByLogin("user")).thenReturn(user);
		when(securityMediator.isAuthorizedToEdit(any(Repository.class), eq(user.get()))).thenReturn(false);
	...
	public void deleteRepository() throws Exception {
		...
		mockMvc
			.perform(MockMvcRequestBuilders
					.delete("/api/v2/manager/python/repositories/" + ID))
	...
	public void deleteRepository_returns401_whenUserIsNotAuthenticated() throws Exception {
		mockMvc
			.perform(MockMvcRequestBuilders
					.delete("/api/v2/manager/python/repositories/" + 123))
			.andExpect(status().isUnauthorized());
	}
	...
	public void deleteRepository_returns500_whenRepositoryDeletionFails() throws Exception {
		...
		when(pythonRepositoryService.findById(ID)).thenReturn(Optional.of(repository));
		doThrow(new DeleteEntityException())
			.when(pythonRepositoryDeleter).delete(any());
		when(userService.findByLogin("user")).thenReturn(user);
		when(userService.isAdmin(user.get())).thenReturn(true);
