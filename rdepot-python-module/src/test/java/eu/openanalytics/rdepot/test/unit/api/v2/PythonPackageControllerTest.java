...
public class PythonPackageControllerTest extends ApiV2ControllerUnitTest{
	...
	public void getAllPackages_returns401_whenUserIsNotAuthenticated() throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/v2/manager/python/packages").contentType(MediaType.APPLICATION_JSON))
	...
	public void getPackage_returns401_whenUserIsNotAuthenticated() throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/v2/manager/python/packages/" + 123).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isUnauthorized())
				.andExpect(content().json(Files.readString(Path.of(ERROR_NOT_AUTHENTICATED_PATH))));
	}
	...
	public void getAllDeletedPackages_returns403_whenUserIsNotAdmin() throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/v2/manager/python/packages").param("deleted", "true")
	...
	public void getAllPackages() throws Exception {
		...
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/v2/manager/python/packages")
	...
	public void getAllPackages_WhenRepositoryIsSpecified() throws Exception {
		...
		when(pythonRepositoryService.findByName(repository.getName())).thenReturn(Optional
				.of(PythonRepositoryTestFixture.GET_EXAMPLE_REPOSITORY()));
		when(userService.findByLogin("user")).thenReturn(user);
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/api/v2/manager/python/packages")
				.param("repositoryName", repository.getName()).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andExpect(content().json(Files.readString(Path.of(EXAMPLE_PACKAGES_REPO_PATH))));
	}
	...
	public void getAllPackages_OnlyDeleted() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/api/v2/manager/python/packages").param("deleted", "true")
	...
	public void getPackageById_AsAdmin() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/api/v2/manager/python/packages/" + packageBag.get().getId())
				.contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
	...
	public void getPackageById_returns404_WhenPackageIsNotFound() throws Exception {
		...
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/v2/manager/python/packages/" + ID).contentType(MediaType.APPLICATION_JSON))
	...
	public void patchPackage_returns404_WhenPackageIsNotFound() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.patch("/api/v2/manager/python/packages/" + ID).content(patchJson)
				.contentType("application/json-patch+json")).andExpect(status().isNotFound())
				.andExpect(content().json(Files.readString(Path.of(EXAMPLE_PACKAGE_NOT_FOUND_PATH))));
	}
	...
	public void patchPackage_returns401_WhenUserIsNotAuthenticated() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.patch("/api/v2/manager/python/packages/" + 123).content(patchJson)
				.contentType("application/json-patch+json")).andExpect(status().isUnauthorized())
				.andExpect(content().json(Files.readString(Path.of(ERROR_NOT_AUTHENTICATED_PATH))));
	}
	...
	public void patchPackage() throws Exception {
		...
		Strategy<PythonPackage> strategy = Mockito
				.spy(new SuccessfulStrategy<PythonPackage>(packageBag, newsfeedEventService, pythonPackageService, user.get()));
		when(pythonStrategyFactory.updatePackageStrategy(any(), eq(user.get()), any())).thenReturn(strategy);
		...
		doAnswer(new Answer<>() {
			...
			public Object answer(InvocationOnMock invocation) throws Throwable {
				ValidationResult validationResult =
						invocation.getArgument(2, ValidationResult.class);
				validationResult.error("MULTIPART-FILE", MessageCodes.INVALID_FILENAME);
				return null;
			}
		}).when(pythonPackageValidator).validate(any(), eq(true), any());
	...
	public void patchPackage_returns500_whenErrorIsThrown() throws Exception {
		...
		Submission submission = PythonSubmissionTestFixture.GET_FIXTURE_SUBMISSION(user.get(), packageBag);
		Strategy<PythonPackage> strategy = Mockito
				.spy(new FailureStrategy<PythonPackage>(packageBag, newsfeedEventService, pythonPackageService, user.get()));
		...
		ResultActions result = mockMvc.perform(MockMvcRequestBuilders.patch("/api/v2/manager/python/packages/" + packageBag.getId())
				.content(patchJson).contentType("application/json-patch+json"))
	 ...
	 packageBag.setDeleted(true);
		...
		when(userService.findByLogin("user")).thenReturn(user);
		doThrow(new DeleteEntityException()).when(pythonPackageDeleter).deleteAndSynchronize(packageBag);
		when(pythonPackageService.findById(packageBag.getId())).thenReturn(Optional.of(packageBag));
		
	 when(pythonPackageService.findOneDeleted(packageBag.getId()))
	 .thenReturn(Optional.of(packageBag));
	
	 ResultActions result = mockMvc
	 .perform(MockMvcRequestBuilders
	 .delete("/api/v2/manager/python/packages/" + packageBag.getId()))
	 .andExpect(
		 status()
