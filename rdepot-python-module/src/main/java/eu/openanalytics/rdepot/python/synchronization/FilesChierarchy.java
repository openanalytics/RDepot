...
@NoArgsConstructor
public class FilesChierarchy {
	private final static String separator =  FileSystems.getDefault().getSeparator();
	
	public static boolean isRepositoryDir(File file) {
		File repositoryIndex = new File(file.getParentFile().getParentFile().getAbsolutePath() + separator + "index.html");
	...
	public static String getArchiveDirectory(File file, int chunkNo) {
		...
		if(!isRepositoryDir(file)) {
			archiveDirectory = new File(file.getParentFile().getParentFile().getParentFile().getAbsolutePath()  + separator + chunkNo);
		} else {
