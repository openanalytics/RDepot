...
public class PythonRepositorySynchronizer implements RepositorySynchronizer<PythonRepository>{
	...
	public final static Comparator<PythonPackage> PACKAGE_COMPARATOR 
			= Comparator.comparingInt(PythonPackage::getId);
	
	@Override
	public void storeRepositoryOnRemoteServer(
			PythonRepository repository, String dateStamp) throws SynchronizeRepositoryException {
		LinkedHashSet<PythonPackage> packages = new LinkedHashSet<>(
	...
	private synchronized void storeRepositoryOnRemoteServer(PythonRepository repository, 
			String dateStamp, List<PythonPackage> packages) 
	...
	private void synchronizeRepository(
			...
			PythonRepository repository) throws SynchronizeRepositoryException {
		...
		ResponseEntity<String> response = rest.getForEntity(serverAndPort + "/python/" + repositoryDirectory + "/", String.class);
		...
		} catch(SendSynchronizeRequestException  | IOException e) {
	...
	private void sendSynchronizeRequest(SynchronizeRepositoryRequestBody request,
			String serverAddress, String repositoryDirectory) throws SendSynchronizeRequestException, IOException {
		...
		try {
			...
			for(MultiValueMap<String, Object> chunk : chunks) {
				...
				ResponseEntity<RepoResponse> httpResponse = 
						rest.postForEntity(serverAddress + "/python/" + repositoryDirectory, entity, RepoResponse.class);
			...
			IntStream.range(0,  chunksSize).forEachOrdered(chunkNo -> {
				String dirToRemovePath = FilesChierarchy.getArchiveDirectory(request.getFilesToUpload().get(0) , chunkNo);
				File dirToRemove = new File(dirToRemovePath);
				if(dirToRemove.exists()){
				FilesChierarchy.deleteDirectory(dirToRemove);
				}
			});
		}
	}
